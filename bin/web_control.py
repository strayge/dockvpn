...
hashed_key = base64.b64encode(f'{USERNAME}:{PASSWORD}'.encode()).decode()
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def check_auth(self):
        if self.headers.get('Authorization') == f'Basic {hashed_key}':
            return True
        else:
            self.do_AUTHHEAD()
            return False
    ...
    def do_GET(self):
        ...
        try:
            ...
            if 'generate' in params and params['generate'][0]:
                self._set_headers()
                self.write("<br /><a href='/'>Main page</a><br/><br/>")
                name = params['generate'][0]
                if not check_name(name):
                    self.write('not allowed character in name (allowed: %s)' % ALLOWED_CHARS)
                    return
                try:
                    out = subprocess.check_output(["generate_client", name]).decode()
                    self.write("<pre>" + out + "</pre>")
                except:
                    self.write("Error =(")
            # remove client
                ...
                if not check_name(name):
                    ...
                    return
                try:
                    out = subprocess.check_output(["revoke_client", name]).decode()
                    self.write("<pre>" + out + "</pre>")
                except:
                    self.write("Error =(")
            # download ovpn config
                ...
                if 'type' not in params or params['type'][0] not in ['tcp', 'udp']:
                    self._set_headers()
                    self.write("<br /><a href='/'>Main page</a><br/><br/>")
                    self.write('wrong type specified')
                    return
                proto = params['type'][0]
